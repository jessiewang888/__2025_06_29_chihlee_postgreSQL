...
ORDER BY SUM(p."進站人數") DESC;
  ...
  */
  ...
  SELECT
    t."name" AS "車站名稱",
    EXTRACT(YEAR FROM p."日期") AS "年份",
    SUM(p."進站人數") AS "每年進站人數"
FROM "每日各站進出站人數" p
LEFT JOIN "台鐵車站資訊" t ON p."車站代碼" = t."stationCode"
WHERE t."name" = '基隆'
  AND EXTRACT(YEAR FROM p."日期") IN (2020, 2021, 2022)
GROUP BY t."name", EXTRACT(YEAR FROM p."日期")
ORDER BY "年份";
...
ORDER BY t."name", "年份";
  ...
  */
  ...
  SELECT
    t."name" AS "車站名稱",
    AVG(p."進站人數") AS "2022年平均每日進站人數"
FROM "每日各站進出站人數" p
LEFT JOIN "台鐵車站資訊" t ON p."車站代碼" = t."stationCode"
WHERE EXTRACT(YEAR FROM p."日期") = 2022
GROUP BY t."name"
HAVING AVG(p."進站人數") > 20000
ORDER BY "2022年平均每日進站人數" DESC;